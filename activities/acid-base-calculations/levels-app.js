(function () {
  ...
  function renderReadOnlyReview(reviewed) {
    ...
    elements.questionPanel.innerHTML = `<header class="question-header"><div><p class="question-meta">READ-ONLY REVIEW</p><h3>${notation(reviewed.templateLabel)}</h3></div></header><p>${notation(reviewed.intro)}</p>${rows}<div class="response-list">${targets}</div>`;
