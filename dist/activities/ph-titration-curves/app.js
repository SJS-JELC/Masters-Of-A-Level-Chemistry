(function(root){
  ...
  let reviewing=false;
  let activeLevel=Number(new URLSearchParams(root.location?.search||'').get('level'))===3?3:2;
  const levelQuestions=()=>questions.filter(q=>q.level===activeLevel);
  ...
  function questionMarkup(){
    ...
    document.querySelector('.level-badge').textContent=`LEVEL ${question.level}`;
    document.querySelectorAll('[data-practice-level]').forEach(link=>{link.setAttribute('aria-current',Number(link.dataset.practiceLevel)===question.level?'page':'false');});
  ...
  function renderChart(){
    ...
    grid.append(svgNode('line',{x1:X0,y1:YBOT,x2:X1,y2:YBOT,class:'axis-line'}),svgNode('line',{x1:X0,y1:YTOP,x2:X0,y2:YBOT,class:'axis-line'}));labels.append(text((X0+X1)/2,430,`Volume of ${question.titrantName} added / cm³`,'axis-label'),text(18,(YTOP+YBOT)/2,'pH','axis-label'));labels.lastChild.setAttribute('transform',`rotate(-90 18 ${(YTOP+YBOT)/2})`);
  ...
  function render(){if(!question||!state)return;renderPieces();renderChart();renderReviewId();document.querySelectorAll('[data-indicator]').forEach(button=>button.setAttribute('aria-pressed',String(button.dataset.indicator===state.indicator)));$('undo').disabled=reviewing||!history.length;$('question-counter').textContent=`Question ${levelQuestions().findIndex(q=>q.id===question.id)+1} of ${levelQuestions().length}`;$('curve-status').textContent=reviewing?'Model answer':state.before&&state.after?'Curve ready to check':'Choose both curve halves';$('curve-status').classList.toggle('ready',Boolean(state.before&&state.after));renderFeedback();renderFeedbackDetailed();}
  ...
  root.TitrationActivity={getQuestion:()=>clone(question),getState:()=>clone(state),getReviewId:()=>reviewId(),isReviewing:()=>reviewing,getLevel:()=>activeLevel,setLevel:level=>{if(![2,3].includes(Number(level)))return false;if(activeLevel===Number(level)&&question?.level===Number(level))return true;activeLevel=Number(level);return loadQuestion(levelQuestions()[0].id);},restore:(qid,nextState)=>loadQuestion(qid,nextState),loadQuestion:(qid,nextState)=>loadQuestion(qid,nextState)};
  document.addEventListener('DOMContentLoaded',()=>{bind();const requested=new URLSearchParams(root.location?.search||'').get('review');if(requested){try{loadReview(requested);}catch(error){showReviewError(error);}}else if(levelQuestions().length)loadQuestion(levelQuestions()[0].id);});
})(globalThis);
