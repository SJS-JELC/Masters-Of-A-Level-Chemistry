(async function () {
  ...
  const testBridge = await globalThis.TestModeBridge?.connect() || null;
  ...
  function submitEvent(event) {
    ...
    if (testBridge) { testSubmit(); return; }
    if (session.result) {
  ...
  el.changePractice.addEventListener("click", () => testBridge ? testNext() : showChooser());
  ...
  const generatorFields = ['rng','cycleIndex','nextDirection','directionUses','groupDirectionUses','representationUses','lastSpecies'];
  function testSnapshot() {
    if (!testBridge || !session?.current) return null;
    return {
      version: "electron-configurations-test-v1", leafId: LEAF, level: session.level,
      seed: session.testSeed, question: structuredClone(session.current), response: structuredClone(session.response),
  ...
  function testStateQuestion(state) {
    return state && state.version === "electron-configurations-test-v1" && state.leafId === LEAF && Number(state.level) === requestedLevel && Number.isSafeInteger(state.seed) && state.seed > 0 && state.seed <= 0xffffffff && state.question && typeof state.question.attemptId === "string" ? state.question : null;
  }
  function testValidState(state) {
    const question = testStateQuestion(state);
    if (!question || !state.response || typeof state.response !== "object") return false;
    try {
      const candidate = S.create({ level: requestedLevel, mode: "level", leafId: LEAF }, Number(state.seed) || 1);
      candidate.current = structuredClone(question); candidate.response = structuredClone(state.response);
      candidate.result = state.result == null ? null : structuredClone(state.result);
      candidate.completedCount = candidate.result ? 1 : 0;
      candidate.correctCount = candidate.result?.correct ? 1 : 0;
      candidate.recordedAttempts = candidate.result ? [question.attemptId] : [];
      if (state.generator) for (const key of generatorFields) candidate[key] = structuredClone(state.generator[key]);
      if (candidate.result && (!Number.isFinite(state.completedAt) || state.completedAt <= 0)) return false;
      return S.valid(candidate);
    } catch (_) { return false; }
  }
  function testBuildSession(state, previous) {
    const level = requestedLevel;
    let testSeed = Number(state?.seed);
    if (!Number.isSafeInteger(testSeed) || testSeed <= 0 || testSeed > 0xffffffff) testSeed = testHash(`${testBridge.sessionId || ""}:${testBridge.attemptId || ""}:${LEAF}:${level}`);
    const validState = testValidState(state);
    const stored = validState ? testStateQuestion(state) : null;
    const base = S.create({ level, mode: "level", leafId: LEAF }, testSeed);
    // Preserve the real level engine's cycle: directions, representations,
    // main-group ions, transition elements and matching must all remain eligible.
    const source = validState ? state : testValidState(previous) ? previous : null;
    if (source?.generator) {
      for (const key of generatorFields) base[key] = structuredClone(source.generator[key]);
      if (!stored) { base.current = null; base.result = null; S.next(base); }
    }
  ...
  async function testReport() {
    if (!testBridge || !session?.result || session.testReported) return;
  ...
  async function startTestMode() {
    if (![1, 2, 3].includes(requestedLevel)) { el.questionPanel.innerHTML = "<p>Test question level unavailable.</p>"; el.activityScreen.hidden = false; return; }
    ...
    document.querySelector(".brand > div")?.setAttribute("hidden", "");
    document.querySelector(".teacher-notes")?.setAttribute("hidden", "");
    el.changePractice?.setAttribute("hidden", "");
    el.progressGrid?.setAttribute("hidden", "");
    el.progressSummary?.setAttribute("hidden", "");
    el.activityScreen.hidden = false; el.chooser.hidden = true; el.finishScreen.hidden = true;
    session = testBuildSession(testBridge.state, testBridge.previous);
    showActivity();
    if (session.result) await testReport();
  }
  globalThis.ElectronLevelsApp = { start, showChooser };
  if (testBridge) { await startTestMode(); return; }
